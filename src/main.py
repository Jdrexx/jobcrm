...
def create_application(req: ApplicationRequest):
    ...
    app_id=save_record('application', f"{req.company} - {req.role}", json_dumps(payload)); return {'id': app_id, **payload}
@app.get('/api/applications')
def applications():
    items=[json_loads(r['payload']) | {'id':r['id'],'created_at':r['created_at']} for r in rows('application')]
